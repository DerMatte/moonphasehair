...
function getPhaseBoundaries(lunarAge: number): { start: number; end: number } {
	...
	const boundaries = [
		...
		{ start: 20.30228, end: 23.9936 }, // Last Quarter
		{ start: 23.9936, end: 29.53059 }, // Waning Crescent
	];

	// Handle wrap-around for new moon
	if (lunarAge < boundaries[0].end || lunarAge >= boundaries[7].start) {
		return {
			start: boundaries[7].start,
			end: boundaries[0].end + PHASE_AGES.SYNODIC_MONTH,
		};
	}
...
export function getMoonPhaseWithTiming(date: Date = new Date()) {
	...
	const currentPhaseStart = new Date(
		date.getTime() - daysIntoPhase * 24 * 60 * 60 * 1000,
	);

	const currentPhaseEnd = new Date(
		date.getTime() + daysUntilPhaseEnd * 24 * 60 * 60 * 1000,
	);
